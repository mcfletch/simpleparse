...
SPGenerator.addDefinition(
    ...
    SequentialGroup(
        children=[
            ...
            Range(value='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_0123456789',
...
SPGenerator.addDefinition(
    # ("'",(CHARNOSNGLQUOTE/ESCAPEDCHAR)*,"'")  /  ('"',(CHARNODBLQUOTE/ESCAPEDCHAR)*,'"')
    "literal",
    SequentialGroup(
        children=[
...
SPGenerator.addDefinition(
    # '\\',( SPECIALESCAPEDCHAR / ('x',HEXESCAPEDCHAR) / UNICODEESCAPEDCHAR_16 / OCTALESCAPEDCHAR /   )
    "ESCAPEDCHAR",
    SequentialGroup(
        children=[
            Literal(value="\\"),
...
class Parser(baseparser.BaseParser):
    ...
    def __init__(self, ebnf, prebuilts=(), methodSource=None, definitionSources=()):
        ...
        success, tags, next = self.parse(
            ebnf, self._rootProduction, processor=processor)
        if next != len(ebnf):
...
class SPGrammarProcessor(DispatchProcessor):
        ...
        for tup in sublist:
            ...
            if tup[0] == 'negpos_indicator':
                ...
                self._config_error_on_fail(
                    errorOnFail, (tag, left, tup[1], []), buffer)
            else:
                base = result
    ...
    def seq_group(self, info, buffer):
        ...
        if len(result) == 1:
            ...
            raise ValueError("""SequentialGroup on line %s doesn't have an element-token child! grammar was %s""" %
                             (lines(0, left, buffer), buffer[left:left + 25]))
    ...
    def CHARRANGE(self, info, buffer):
        ...
        if isinstance(first, _unicode) or isinstance(second, _unicode):
            ...
            if not (isinstance(second, _unicode) and isinstance(first, _unicode)):
                raise ValueError('Range %s uses one unicode and one string escape, cannot mix' % (
                    buffer[left:right]))
        else:
            _chr = chr
