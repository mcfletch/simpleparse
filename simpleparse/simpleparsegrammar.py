...
SPGenerator.addDefinition("negpos_indicator", Range(value="+-"))
SPGenerator.addDefinition(
    "lookahead_indicator",
    Literal(value="?"),
)
...
SPGenerator.addDefinition(
    ...
    SequentialGroup(
        children=[
            ...
            Range(
                value="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_0123456789",
                optional=1,
                repeating=1,
            ),
        ]
...
Unparsed:\n%s..."""
                % (lineNumber, next, len(ebnf), ebnf[next : next + 100])
            )
...
class SPGrammarProcessor(DispatchProcessor):
        ...
        for item, tup in zip(children, sublist):
            if isinstance(item, ErrorOnFail):
                ...
                if errorOnFail:
                    ...
                    self._config_error_on_fail(item.errorOnFail, tup, buffer)
                result.append(item)
        if len(result) == 1:
            ...
            raise ValueError(
                """SequentialGroup on line %s doesn't have an element-token child! grammar was %s"""
    ...
    def fo_group(self, info, buffer):
        ...
        if len(children) == 1:
            ...
            return children[0]
        base = FirstOfGroup(children=children)
        return base
    ...
    def CHARRANGE(self, info, buffer):
        ...
        return "".join([_chr(u) for u in range(first, second + 1)])
