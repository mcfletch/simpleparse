...
class SimpleParseGrammarTests(unittest.TestCase):
    ...
    def testCharRange1(self):
        self.doBasicTest(
            "CHARRANGE",
            'a-z',
            (1, [('CHARNOBRACE', 0, 1, [('CHAR', 0, 1, NullResult)]),
                 ('CHARNOBRACE', 2, 3, [('CHAR', 2, 3, NullResult)])], 3),
        )
    ...
    def testLiteralDecorator2(self):
        self.doBasicTest(
            "literal",
            'c"this"',
            (1, [('literalDecorator', 0, 1, NullResult),
                 ('CHARNODBLQUOTE', 2, 6, NullResult)], 7),
        )
    ...
    def testDeclarationSet2(self):
        ...
        assert result[-1] == len(declaration), '''Didn't complete parse of the simpleparse declaration, only got %s chars, should have %s''' % (
